			...
			try
			{
				...
				while (!cancellationToken.IsCancellationRequested)
				{
					...
					if (completedTask == receiveTask)
					{
						...
						{
							// Define variables ONCE for debugging this invalid request
							string receivedContentForDebug = "Error decoding received bytes.";
							int receivedLength = result.Buffer?.Length ?? -1;
							string receivedHex = "N/A"; // Default hex representation

							// Attempt to get Hex representation if buffer exists
							if (result.Buffer != null && result.Buffer.Length > 0)
							{
								receivedHex = System.BitConverter.ToString(result.Buffer, 0, result.Buffer.Length).Replace("-", ""); // Format as hex string
							}

							// Attempt to decode the string
							try
							{
								receivedContentForDebug = Encoding.UTF8.GetString(result.Buffer);
							}
							catch (Exception decodeEx)
							{
								receivedContentForDebug = $"UTF8 Decode Error: {decodeEx.Message}";
							}

							// Log detailed debug information ONCE
							LogWarning($"NETWORK DISCOVERY DEBUG (RAW): Invalid request from {result.RemoteEndPoint}. " +
									   $"Expected Secret: '{secret}', Received Byte Length: {receivedLength}, " +
									   $"Received Bytes (Hex): {receivedHex}, " +
									   $"Attempted Decode: '{receivedContentForDebug}'");
						}
					}
					else
					{
		...
		private async Task SearchForServersAsync(CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				while (!cancellationToken.IsCancellationRequested)
				{
					...
					// Inside NetworkDiscovery.cs -> SearchNetworkAsync or wherever SecretBytes is used for sending
					Debug.Log($"CLIENT DEBUG: Preparing to send search. Secret string: '{secret}', SecretBytes length: {_secretBytes?.Length ?? -1}", this);
					// ... the actual send call follows ...
